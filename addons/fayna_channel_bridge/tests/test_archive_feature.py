...
class ArchiveFeatureTestCase(TransactionCase):
    ...
    def test_archives_old_inactive_conversations(self):
        ...
        recent = self._create_conversation('RecentConv', datetime.now() - timedelta(days=5))

        archived_count = self.env['channel.conversation'].cron_archive_old_conversations()
    ...
    def test_archives_related_discuss_channel(self):
        ...
        old = self._create_conversation('OldWithChannel', datetime.now() - timedelta(days=45))
        self.assertTrue(old.channel_id.active)

        self.env['channel.conversation'].cron_archive_old_conversations()

        self.assertFalse(old.channel_id.active, 'discuss.channel має бути заархівований')

    def test_preserves_history(self):
        """Архівація не видаляє запис — лише ховає (soft-archive)."""
        old = self._create_conversation('KeepHistory', datetime.now() - timedelta(days=90))
        ...
        archived = (
            self.env['channel.conversation'].with_context(active_test=False).browse(conversation_id)
        )
    ...
    def test_respects_configured_days(self):
        ...
        recent = self._create_conversation('Recent10d', datetime.now() - timedelta(days=5))
    ...
    def test_minimum_days_floor(self):
        ...
        recent = self._create_conversation('FloorTest', datetime.now() - timedelta(days=3))
    ...
    def test_invalid_days_falls_back_to_default(self):
        ...
        old = self._create_conversation('InvalidCfg', datetime.now() - timedelta(days=40))
