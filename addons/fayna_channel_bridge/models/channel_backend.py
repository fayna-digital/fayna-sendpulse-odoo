...
class ChannelBackend(models.Model):
    ...
    credentials = fields.Text(
        ...
        groups="fayna_channel_bridge.group_channel_bridge_admin",
        help="JSON with provider tokens/keys. Stored in plain text "
        "(administrators only).",
    )
    ...
    def send_message(self, text, attachment_url=None, provider_user_id=None):
        ...
        if text and len(text) > self._TELEGRAM_TEXT_LIMIT:
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                ok, msg_id, last_err = self._telegram_send_single(
                    piece, att, provider_user_id
                )
                if not ok:
                    all_ok = False
                    _logger.warning(
    ...
    def _meta_send_single(self, text, attachment_url, provider_user_id):
        ...
        try:
            ...
            _logger.error(
                "Channel Bridge: Meta send exception — %s", self._mask_secrets(e)
            )
    ...
    def _viber_send_single(self, text, attachment_url, provider_user_id):
        ...
        try:
            ...
            _logger.error(
                "Channel Bridge: Viber send exception — %s", self._mask_secrets(e)
            )
            return False, None, self._mask_secrets(str(e))
    ...
    def _whatsapp_send_single(self, text, attachment_url, provider_user_id):
        ...
        try:
            ...
            _logger.error(
                "Channel Bridge: WhatsApp send exception — %s", self._mask_secrets(e)
            )
            return False, None, self._mask_secrets(str(e))
    ...
    def _tiktok_send_single(self, text, attachment_url, provider_user_id):
        ...
        try:
            ...
            _logger.error(
                "Channel Bridge: TikTok send exception — %s", self._mask_secrets(e)
            )
            return False, None, self._mask_secrets(str(e))
    ...
    def _telegram_send_single(self, text, attachment_url, provider_user_id):
        ...
        try:
            ...
            resp = requests.post(
                endpoint, json=payload, timeout=self._TELEGRAM_API_TIMEOUT
            )
            _logger.info(
    ...
    def action_retry_check(self):
            ...
            if url:
                ...
                return {
                    ...
                    "params": {
                        ...
                        "message": _(
                            "The channel is working. The webhook was re-registered."
                        ),
                        "type": "success",
                        "sticky": False,
                    },
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Availability check for this channel is not implemented yet."
                ),
                "type": "warning",
                "sticky": False,
            },
