## 17.0.1.15.14
- **Фікс: SerializationFailure на `connect.write()` у `_process_incoming_event` рвав усю webhook-транзакцію.** Знайдено під час розбору тижневого Sentry-звіту (26.07.2026, `campscout` prod, ~8 разів/тиждень): два webhook-и по тому самому `contact_id`+`service` (напр. дубльована доставка від SendPulse за секунду) — advisory-lock (`pg_advisory_xact_lock`) вище блокує лише дублі-`create`, не цей `write` на `last_message_preview`/`last_message_date`/`stage`/`sp_messenger_window_expires_at`: другий worker прокидається зі своїм старим snapshot і ловить `could not serialize access due to concurrent update`, що (без savepoint) абортує всю транзакцію запиту — каскадом валиться і наступний `discuss_channel_member`-update, webhook повертає 500. На відміну від `message_count` (`sendpulse_connect.py`, вже мав savepoint+skip з минулого інциденту) тут дані НЕ косметичні — тому не пропускаємо, а ретраїмо `write` один раз усередині `cr.savepoint()`: конкурент до цього моменту вже закомітився, повторний запис проходить чисто без 500/ретраю з боку SendPulse.

## 17.0.1.15.13
- **UX-аудит SendpulseInfoPanel (Discuss OWL-панель), 21.07.2026.** Живий Playwright-огляд staging після рефакторингу знайшов реальні проблеми, код-рев'ю сам по собі не показав:
  - SMS-купон: поле реально показувало явно биту prefill-адресу (`901`), кнопка «Надіслати» лишалась активною — тепер prefill відкидається якщо цифр менше 9, кнопка дизейблена доки номер не схожий на валідний, з попередженням.
  - PDF-каталог / SMS-купон — реальний зовнішній send (email/SMS + списання купона з обмеженого пулу) тепер питає підтвердження (`ConfirmationDialog`) перед відправкою.
  - «Оновити профіль» (іконка в шапці панелі) — раніше без loading-індикатора й без сповіщення про помилку; тепер спінер + toast success/danger, як і решта кнопок панелі.
  - Екран «Помилка завантаження» — додано кнопку «Спробувати ще раз» замість змушувати перемикати канал.
  - Немапований код помилки лід-магніту більше не показує сирий технічний рядок — generic «Сталася помилка, спробуйте пізніше».
  - Іконки-кнопки в шапці панелі (Оновити/Картка) отримали видимий текстовий підпис, не лише hover-title.
  - Мова контакту показувалась сирим ISO-кодом (`uk`) без контексту — тепер людська назва з title-тултипом.

## 17.0.1.15.12
- **Фікс: клієнтське "/lead ..." більше не проковтується як команда оператора.** `mail_channel.py.message_post`: перевірка slash-команди `/lead` спрацьовувала ДО перевірки контексту `sendpulse_incoming` — клієнтське inbound-повідомлення, що випадково починається з "/lead", тихо ковталось (ніколи не потрапляло в чат) і замінювалось авто-створеним CRM-лідом. Знайдено побічно під час sudo()-аудиту 21.07.2026 (перевіряючи caller-шлях цього методу). Порядок перевірок переставлено, `/lead`-парсинг тепер лише для оператора. +2 regression-тести.

## 17.0.1.15.11
- **Odoo/OCA-аудит 19.07.2026, пункти 2 і 8 довершено** («345», продовження Фази 3). Ці два пункти лишились поза фазованим планом рефакторингу (Фаза 0/1/2/3 брали в роботу лише #4/#7/#5/#9/#6/#3) і не були закриті, поки власник не запитав про повноту.
- **#2 sudo()-дисципліна**: повний аудит усіх 110 викликів `.sudo()` у `models/*.py` (5 паралельних агентів по кластерах файлів, кожен трасував caller до контролера/крону/RPC і звіряв з `security/ir.model.access.csv`+`security/security.xml`). Знайдено й прибрано 9 неviправданих ескалацій — усі за одним патерном: `.sudo()` на внутрішньо-автентифікованому шляху (RPC з Discuss-панелі, кнопка форми, wizard), де власних прав викликача вже достатньо, і сусідній код у тому ж файлі робить той самий запис БЕЗ sudo (доказ, що ескалація зайва): `res_config_settings.py:action_sync_fb_pages`, `sendpulse_ai_assist.py:_try_extract_email_and_link` (2 місця), `sendpulse_connect.py:get_connect_for_channel`+`unarchive_partner_for_channel`, `sendpulse_identify_wizard.py:action_create_and_link` (2 місця), `sendpulse_lead_magnet.py:_send_pdf_catalog_email`+`_generate_and_send_sms_coupon`. Решта 101 — виправдані (webhook/крон-контекст без request user, або `ir.config_parameter`/append-only RODO-log де officer не має власних прав). Живий тест підтвердив: 0 нових AccessError на жодному з 42 тестів.
- **#8 magic numbers**: 39 inline-літералів (requests-таймаути, truncation-ліміти рядків, drip/messenger-window порогові години) винесено в named-константи за існуючим патерном файлу (`_ALLOWED_MEDIA_DOMAINS`-стиль) у 14 файлах — чисте перейменування, значення не змінені.
- Живий Docker-тест (fresh `-i`, не `-u`): `Registry loaded`, `Modules loaded`, 0 failed/0 error з 42 тестів, нуль AccessError/AttributeError/TypeError.

## 17.0.1.15.10
- **God Object рефакторинг завершено (Odoo/OCA-аудит 19.07.2026, пункт 4/найважливіше, «345»).** `models/sendpulse_connect.py` розбито з 5486 рядків на Core (~1170 рядків: поля/ORM/Discuss-лайфсайкл) + 14 `_inherit`-шардів без нового `_name` — той самий офіційний Odoo/OCA-патерн, що вже застосований у `mail_channel.py`/`res_partner.py` цього репо: `sendpulse_oauth.py`, `sendpulse_telegram_notify.py`, `sendpulse_rodo.py`, `sendpulse_crm_lead.py`, `sendpulse_drip.py`, `sendpulse_identification.py`, `sendpulse_profile_sync.py`, `sendpulse_lead_magnet.py`, `sendpulse_ai_assist.py`, `sendpulse_meta_api.py`, `sendpulse_comment_autoreply.py`, `sendpulse_reporting.py`, `sendpulse_webhook.py`, `sendpulse_messaging.py`. Кожен крок — механічний перенос, незалежно верифікований (diff/ast.parse/grep/byte-for-byte) і живим Docker-тестом (42/42 на кожному кроці), задеплоєний на staging окремо. Зовнішня поверхня (view-кнопки, JS RPC, 10 `ir.cron` записів, `security`/`ir.model.access.csv`, виклики з `mail_channel.py`/`controllers/`) не зачеплена — усе резолвиться по імені моделі, не по файлу. План: `docs/REFACTOR_PLAN.md`.
- Дорогою знайдено й виправлено 3 реальні баги (не гіпотетичні, задокументовані в `docs/REFACTOR_PLAN.md`/`claude-memory/project_sendpulse_refactor_brief_345_2026-07-20`): (1) `_check_and_record_unsubscribe` звертався до неіснуючого поля `self.sp_contact_id` (мало бути `sendpulse_contact_id`) — неідентифікований контакт, що пише unsubscribe-фразу, валив `AttributeError`; (2) клас двічі визначав `_EMAIL_REGEX` (AI-assist з `\b`-межами, Identification без меж) — друге визначення тихо перекривало перше для всього класу; (3) `_is_allowed_media_url` мав захардкоджене посилання на ім'я Python-класу `SendpulseConnect._ALLOWED_MEDIA_DOMAINS` — після переносу в шард впало б `AttributeError`, спіймано й виправлено до деплою.
- `models/__init__.py`: `sendpulse_connect` (визначає `_name='sendpulse.connect'`) тепер завжди імпортується ПЕРШИМ серед `sendpulse_*`-записів — Odoo будує registry в порядку Python-імпорту, `_inherit`-шард без власного `_name` мусить іти після файлу, що цей `_name` визначає. Захищено від lint-автофіксу окремим `pyproject.toml`-винятком (`ruff` `I001` ignore на цей файл).

## 17.0.1.15.9
- **Odoo/OCA-стандарт манiфесту** (пункт 5 аудиту 19.07.2026, довершено): `depends` тепер включає `sale` — модуль реально посилається на `sale.sale_menu_root` (`wizards/oboz300_quick_send_views.xml:46`), відсутність у `depends` призводила до `ParseError` при fresh-install модуля наодинці (на проді/стейджингу не проявлялось, бо `sale` вже встановлений транзитивно). Знайдено живим Docker-тестом під час рефакторингу «345».
- **Версія переведена на 5-частинний OCA-формат** (`{odoo_serie}.{major}.{minor}.{patch}`): `17.0.15.8` → `17.0.1.15.9`. Історичні номери (15, 8→9) збережено як minor.patch, додано константний major=1.
...
## 17.0.15.5
- `controllers/main.py`: обробка події (`_process_incoming_event`/`_process_outgoing_event`/`_process_unsubscribe`) тепер в окремому `cr.savepoint()`, ПІСЛЯ вставки `sendpulse.webhook.data`. Раніше будь-який виняток під час обробки відкочував ВСЮ транзакцію запиту, включно з уже вставленим сирим audit-записом — тепер audit виживає незалежно від успіху обробки й доступний для ручного backfill. Доповнює фікс v17.0.15.3 (savepoint на `message_count`), який захищав лише один конкретний рядок — цей шар захищає взагалі всю обробку від будь-якого майбутнього збою того самого класу.

## 17.0.15.4
- `sendpulse.webhook.data.clear_old_webhooks`: ретеншен 7→30 днів. Саме завдяки цій таблиці вдалось перевірити 7-денне вікно «що SendPulse реально надіслав vs що є в Odoo» і спіймати помилку в діагностичному SQL (JOIN-фанаут завищував розрив у ~3 рази) — коротка ретенція обмежувала глибину майбутніх таких аудитів.
...
### Додано
- **FAQ Entry — перекладні дані:** `translate=True` на `name`/`question`/`answer` — стандартний механізм Odoo: на записі з'являється перемикач мов (вписуєш PL/UA версії, при зміні мови інтерфейсу показується відповідна). Існуючий вміст стає source (en_US) і показується як fallback, доки переклад не введено.
- **Захист RAG:** `get_active_faq_for_prompt` пінується на `lang='en_US'` (source) — база знань бота не залежить від мови оператора, що викликав RAG; поведінка бота незмінна.

## [17.0.14.8] — 2026-06-12

### Додано
- **i18n:** каталог `i18n/` з `pl.po` + `uk_UA.po` (654 терміни кожна, 100%) — інтерфейс модуля автоматично перемикається за мовою користувача Odoo (PL/UA). Згенеровано канонічно: `odoo --i18n-export` .pot з прода (з occurrence-коментарями) → агентний переклад з незалежною рецензією → msgfmt PASS, плейсхолдери 0 розбіжностей. Тексти бота клієнтам не зачеплено.
- **fix:** порт hotfix з прода — параметр `odoo_chatwoot_connector.greeting_enabled` (kill-switch авто-привітання); жив лише на сервері як брудний файл (#4ZONES).
- 3 display_notification обгорнуто в `_()`.
